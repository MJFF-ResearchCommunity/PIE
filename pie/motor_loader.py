...
def load_ppmi_motor_assessments(folder_path: str) -> pd.DataFrame:
    ...
    :param folder_path: Path to 'Motor___MDS-UPDRS' folder containing CSV files.
    ...
    if not found_any_file or df_merged is None:
        print("[WARNING] No matching motor assessment CSV files were successfully loaded. Returning empty DataFrame.")
        return pd.DataFrame()
