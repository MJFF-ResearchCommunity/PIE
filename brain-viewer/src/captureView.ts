...
export function captureLines(
  ...
  return [
    ...
    `${d.mode} · ${boundaries ? "segmentation boundaries (not validated pial surfaces)" : (metric?.name ?? "measured image")} · window ${d.window.map((x) => x.toPrecision(4)).join("–")} ${metric?.units ?? p.scan.units}`,
    ...
    boundaries
      ? `Anatomical label colors · shells L ${Math.round(d.leftOpacity * 100)}%, R ${Math.round(d.rightOpacity * 100)}% · selected labels ${d.selectedStructures.join(", ")}`
